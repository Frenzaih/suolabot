    ...
    collector.on('collect', async i => {
      if (i.customId === 'cancel') {
        ...
        const opponentPrompt = await interaction.followUp({
          content: `${player1.username} challenges you to gamba ${betAmount} money. Do you want to proceed or pussy out?`,
          components: [acceptRow],
          ephemeral: true // Make this message ephemeral
        });

        const opponentFilter = i => i.user.id === player2.id;
        const opponentCollector = interaction.channel.createMessageComponentCollector({ filter: opponentFilter, time: 60000 });
