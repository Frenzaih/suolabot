      ...
      if (i.customId === 'cancel') {
        ...
        opponentCollector.on('collect', async i => {
          if (i.customId === 'refuse') {
            await i.update({ content: 'Refused. What a pussy...', components: [] });
            ...
            await i.update({ content: 'Gamba accepted!', components: [] });

            const gameStartMessage = `${player1.username} bets ${betAmount} money against ${player2.username} in a game of dice!`;
            const gameMessage = await interaction.channel.send({ content: gameStartMessage });
