...
// Module exports
module.exports = {
  // Show player's points and money
  showPlayerPointsAndMoney: function (userId) {
    const json = readPlayerData();
    const playerIndex = findPlayerIndex(userId, json.players);

    if (playerIndex !== -1) {
      console.log(`Found player ${json.players[playerIndex].name} with ${json.players[playerIndex].points} points and ${json.players[playerIndex].money} money.`);
      return {
        id: json.players[playerIndex].id,
        name: json.players[playerIndex].name,
        points: json.players[playerIndex].points,
        money: json.players[playerIndex].money
      };
    } else {
  ...
  // Update player's points and money
  updatePointsAndMoney: function (userId, userName, pointsToAdd, moneyToAdd) {
    const json = readPlayerData();
    const playerIndex = findPlayerIndex(userId, json.players);

    if (playerIndex !== -1) {
      // Update the player's points, money, and name
      json.players[playerIndex].points += pointsToAdd;
      json.players[playerIndex].money += moneyToAdd;
      json.players[playerIndex].name = userName; // Update the name to the current name
      console.log(`Updated player ${json.players[playerIndex].name}'s points to ${json.players[playerIndex].points} and money to ${json.players[playerIndex].money}.`);
    } else {
      // Store userId, current name, points, and money for new user
      json.players.push({ id: userId, name: userName, points: pointsToAdd, money: moneyToAdd });
      console.log(`Added new player ${userName} with ${pointsToAdd} points and ${moneyToAdd} money.`);
    }
  ...
  leaderboard: function () {
    ...
    const leaderboardEntries = sortedPlayers.map(player => 
      `${player.name} - ${player.points} points, ${player.money} money`
    );
  ...
  handleDuckMessage: async function (message) {
    ...
    if (message.content.includes('🦆')) {
      ...
      // Determine points and money based on the order of commenting
      let pointsToAdd = 0;
      let moneyToAdd = 0;
      ...
      // Update points and money for the user, using the current username
      this.updatePointsAndMoney(userId, userName, pointsToAdd, moneyToAdd); // Add points and money based on order
