...
async function checkAndPostIfSilent(client, duck_channel, postTime, checkCount = 0) {
  ...
  // Calculate the remaining time, capped at 5 minutes
  const remainingTime = Math.min(5, (5 * 60 - silenceTime) / 60);
  ...
  setTimeout(async () => {
    // Check if the current time is past 20 PM
    if (moment().hour() >= 20) {
      console.log("It's past 8 PM. Forcefully posting the duck image.");
      await postDuckImage(client, duck_channel);
    } else if (checkCount >= 3) {
      console.log("Maximum checks reached. Forcefully posting the duck image.");
      await postDuckImage(client, duck_channel);
    } else {
      // Recursively call the function with the updated check count
...
// Function to handle duck images on bot startup
async function onBotStart(client, duck_channel) {
  ...
  if (!hasPostedToday()) {
    ...
    const startMin = Math.max(600, currentTotalMinutes + 1); // 10 AM (600) or 1 minute from now
    const endMin = 1200; // 8 PM (1200)

    // Check if current time is past 8 PM (1200 minutes)
    if (startMin >= endMin) {
      console.log(
        `It's past 8 PM (Current time: ${now.format(
          "DD-MM-YYYY HH:mm:ss"
        )}). Posting immediately if the channel is silent.`
      );
      await checkAndPostIfSilent(client, duck_channel, now);
    } else {
      // Randomize total minutes within the available window
      const randomMinutes =
        Math.floor(Math.random() * (endMin - startMin)) + startMin;

      // Set the time for posting by adding random minutes to the start of today
      const postTime = now.clone().startOf("day").add(randomMinutes, "minutes");

      // Log the current time and the selected posting time for troubleshooting
      console.log(
        `Current time: ${now.format(
          "DD-MM-YYYY HH:mm:ss"
        )} | Duck image scheduled for: ${postTime.format(
          "DD-MM-YYYY HH:mm:ss"
        )}`
      );
      ...
      schedule.scheduleJob(postTime.toDate(), async () => {
        // Check if the channel is silent.
        console.log(
          `Checking silence for the scheduled post at: ${postTime.format(
            "DD-MM-YYYY HH:mm:ss"
          )}`
        );
...
function scheduleDailyDuckImagePosting(client, duck_channel) {
  ...
  schedule.scheduleJob('59 9 * * *', async () => {
    ...
    // Generate a random time between 10 AM and 8 PM
    const randomHour = Math.floor(Math.random() * 10) + 10; // Random hour between 10 and 20
