...
async function onBotStart(client, duck_channel) {
  ...
  if (!hasPostedToday()) {
    ...
    // If the current time is past 8 PM but before 12 PM, post immediately. This is just in case the bot has been offline the whole day
    // or something similar. Less chance of pitchforks since there was no duck during the correct time.
    if (now.hour() > 20 && now.hour() <= 24) {
      console.log("It's past 8 PM but before 12 PM. Posting immediately if the channel is silent.");
      await checkAndPostIfSilent(client, duck_channel, now);
    } else {
      // Randomize time between current time and 8 PM
      const minHour = Math.max(10, now.hour()); // Ensure at least 10 AM or current hour if later
      const randomHour = Math.floor(Math.random() * (20 - minHour)) + minHour; // Random hour between minHour and 8 PM
