...
const duckImagesDir = path.join(__dirname, '../../data/duck-images');
const usedImagesPath = './data/usedDuckImages.json';
const activationDays = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']; // Specify active days
...
async function checkChannelSilence(channel) {
...
async function postDuckImage(client, markAsUsed = true) {
  const duckChannel = client.channels.cache.get(process.env.DUCK_CHANNEL);

  if (!duckChannel) {
    console.error('Duck channel not found. Please check the channel ID.');
    return;
  }
...
async function checkAndPostIfSilent(client, duck_channel, postTime, checkCount = 0) {
  ...
  if (silence) {
    ...
    const remainingTime = 5 * 60 * 1000 - silenceTime; // Remaining time in milliseconds
...
function scheduleDailyDuckImagePosting(client, duck_channel) {
  ...
  schedule.scheduleJob('59 9 * * *', async () => {
    ...
    const randomHour = Math.floor(Math.random() * 7) + 10; // Random hour between 10 and 17
    const randomMinute = Math.floor(Math.random() * 60); // Random minute
